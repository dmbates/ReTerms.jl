...
SimpleScalarReTerm(f::PooledDataVector) = SimpleScalarReTerm(f,1.)
...
function Base.Ac_mul_B!(r::VecOrMat, t::SimpleScalarReTerm, v::VecOrMat)
    ...
    if k == 1
        for i in 1:n
            r[i] = t.λ * v[t.f.refs[i]]
        end
    else
        for j in 1:k, i in 1:n
            r[i,j] = t.λ * v[t.f.refs[i],j]
        end
    end
    r
end
...
PDMats.PDMat(t::SimpleScalarReTerm) = PDiagMat(t.λ * (t * ones(size(t, 2))))  # Λ'Z'Z*Λ
